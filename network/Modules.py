...
class Refine(nn.Module):
    ...
    def forward(self, f, pm):
        ...
        m = s + \
            F.interpolate(pm, scale_factor=self.scale_factor, mode='bilinear')
...
class RefineSimple(nn.Module):
    def __init__(self, inplanes, planes, scale_factor=2):
        ...
        self.convMM1 = nn.Conv3d(
            planes + inplanes, planes, kernel_size=3, padding=1)
        self.convMM2 = nn.Conv3d(planes, planes, kernel_size=3, padding=1)
        self.scale_factor = scale_factor

    def forward(self, f, pm):
        m = torch.cat(
            (f, F.interpolate(pm, size=f.shape[-3:], mode='trilinear')), dim=1)
...
class Refine3dConvTranspose(nn.Module):
    def __init__(self, inplanes, planes, scale_factor=2):
        ...
        self.conv_t = nn.ConvTranspose3d(
            planes, planes, 2, stride=2, bias=True)
        self.scale_factor = scale_factor
...
class SoftmaxSimilarity(nn.Module):
    ...
    def forward(self, x):
        ...
        shape = m1.shape
        m1 = m1.permute(0, 2, 3, 1).view(
            m1.shape[0], m1.shape[2] * m1.shape[3], m1.shape[1])
        m2 = m2.permute(0, 2, 3, 1).view(
            m2.shape[0], m2.shape[2] * m2.shape[3], m2.shape[1]).transpose(1, 2)
        ...
        x = output.transpose(1, 2).view(
            shape[0], shape[2] * shape[3], shape[2], shape[3])
        del m1, m2
        return x
...
class _ASPPConv(nn.Sequential):
    def __init__(self, in_planes, out_planes, dilation):
        super(_ASPPConv, self).__init__(
            nn.Conv3d(in_planes, in_planes, (1, 3, 3), padding=(0, dilation, dilation), dilation=(
                1, dilation, dilation), groups=out_planes, bias=False),
            nn.ReLU(inplace=True),
...
class ASPPModule(nn.Module):
    def __init__(self, in_planes, out_planes, inter_planes=None):
        ...
        self.conv = nn.Conv3d(inter_planes * 5, out_planes,
...
class ChannelSepConv3d(nn.Sequential):
    def __init__(self, inplanes, outplanes, n_groups=32):
        super(ChannelSepConv3d, self).__init__(
            nn.Conv3d(inplanes, inplanes, 3, padding=1,
                      groups=outplanes, bias=False),
...
class BMVC19Decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv1(x1)
        x = F.interpolate(x, x2.shape[-3:],
                          mode='trilinear', align_corners=True)
        x = torch.cat((x, x2), 1)

        x = self.conv2(x)
        x = F.interpolate(x, x3.shape[-3:],
                          mode='trilinear', align_corners=True)
        x = torch.cat((x, x3), 1)
