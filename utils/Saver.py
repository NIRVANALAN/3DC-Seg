...
def load_weightsG(net_G, optim_G, wts_file, model_dir):
    start_epoch = 0
    start_iter = 0
    ...
    if checkpoint is not None:
        ...
        print('Loaded weights G from {}'.format(load_name))

    return net_G, optim_G, start_epoch, start_iter
...
def load_weightsV2(model, optimiser, wts_file, model_dir):
    ...
    if checkpoint is not None:
        ...
        missing_keys = np.setdiff1d(
            list(state.keys()), list(checkpoint_valid.keys()))
    ...
    if loadepoch is not None:
        ...
        if len(chkpts) > 0:
            ...
            load_name = os.path.join(
                "saved_models", '{}.pth'.format(loadepoch))
            state = model.state_dict()
            ...
            load_name = {1: 'saved_models/youtubevos_pretrain.pth',
                         2: 'saved_models/rgmp.pth'}
            ...
            start_epoch = checkpoint['epoch'] + \
                1 if (args.task == "train" and "epoch" in checkpoint) else 0
...
def save_checkpointG(epoch, iter, net_G, optim_G, save_name):
    torch.save({'epoch': epoch,
...
def save_checkpointV3(epoch, iter, net_G, net_D, optim_G, optim_D, save_name):
    torch.save({'epoch': epoch,
                'net_G': net_G.state_dict(),
