...
class Encoder3d(nn.Module):
    def __init__(self, backbone, tw, pixel_mean, pixel_std):
        ...
        resnet = get_backbone_fn(backbone.NAME)(
            sample_size=112, sample_duration=tw)
        ...
        self.register_buffer('mean', torch.FloatTensor(
            pixel_mean).view(1, 3, 1, 1, 1))
        self.register_buffer('std', torch.FloatTensor(
            pixel_std).view(1, 3, 1, 1, 1))
...
class Decoder3d(nn.Module):
    def __init__(self, n_classes, inter_block, refine_block, pred_scale_factor=(1, 4, 4)):
        ...
        self.pred5 = nn.Conv3d(
            mdim, n_classes, kernel_size=3, padding=1, stride=1)
        self.pred4 = nn.Conv3d(
            mdim, n_classes, kernel_size=3, padding=1, stride=1)
        self.pred3 = nn.Conv3d(
            mdim, n_classes, kernel_size=3, padding=1, stride=1)
        self.pred2 = nn.Conv3d(
            mdim, n_classes, kernel_size=3, padding=1, stride=1)
    ...
    def forward(self, r5, r4, r3, r2, support):
        ...
        p = F.interpolate(
            p2, scale_factor=self.pred_scale_factor, mode='trilinear')
...
class SaliencyNetwork(BaseNetwork):
    def __init__(self, cfg):
        ...
        self.encoder = Encoder3d(
            cfg.MODEL.BACKBONE, cfg.INPUT.TW, cfg.MODEL.PIXEL_MEAN, cfg.MODEL.PIXEL_STD)
    ...
    def forward(self, x, ref=None):
        ...
        def flatten(lst): return [lst] if type(lst) is torch.Tensor else reduce(
            torch.add, [flatten(ele) for ele in lst])
        p = flatten([decoder.forward(r5, r4, r3, r2, None)
