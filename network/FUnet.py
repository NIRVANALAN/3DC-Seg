...
class FusionUNet(BaseNetwork):
    """Constructs a Depth Fusion Model
    Adapted from https://github.com/pytorch/vision/blob/master/torchvision/models/resnet.py
    """

    def __init__(self, cfg, ngf=64, deepBlender=True):
        super(FusionUNet, self).__init__()  # TODO
        self.up_conv1_inplanes = cfg.MODEL.BACKBONE.UP_CONV1_INPLANES
        self.up_conv1_outplanes = cfg.MODEL.BACKBONE.UP_CONV1_OUTPLANES
        self.deepBlender = cfg.MODEL.BACKBONE.DEEPBLENDER
        assert type(self.up_conv1_inplanes) in [
            tuple, list] and len(self.up_conv1_inplanes) == 5
        assert type(self.up_conv1_outplanes) in [
            tuple, list] and len(self.up_conv1_outplanes) == 5
        ...
        self.model = UNetBlock(down_conv1_in=cfg.MODEL.BACKBONE.INPUT_C, down_conv2_in=ngf,  # * outer most
...
class UNetBlock(nn.Module):
    def __init__(self, down_conv1_in, down_conv2_in, up_conv1_in, up_conv1_out, up_conv2_out, submodule=None, innermost=False, norm_layer=nn.InstanceNorm2d, deepBlender=False):
        ...
        # FIXME
        up_conv1 = nn.Conv2d(up_conv1_in, up_conv1_out, kernel_size=3,
                             stride=1, padding=1)  # TODO, padding size
    ...
    def forward(self, x_left, x_right):
        x_left = self.down(x_left)
        x_right = self.down(x_right)
        identity_x_ds = x_right
        identity_x_dm = x_left
        if not self.innermost:
            output = self.submodule(self.maxpool(
                x_left), self.maxpool(x_right))
        else:
            output = torch.cat(
                [self.maxpool(identity_x_ds), self.maxpool(identity_x_dm)], 1)
