...
def get_lr_schedulers(optimiser, cfg, last_epoch=-1):
    ...
    if 'exponential' in cfg.SOLVER.LR_SCHEDULERS:
        lr_schedulers += [torch.optim.lr_scheduler.ExponentialLR(
            optimiser, gamma=cfg.SOLVER.GAMMA, last_epoch=last_epoch)]
...
def get_lr_schedulers_args(optimiser, args, last_epoch=-1):
    ...
    if 'exponential' in args.lr_schedulers:
        lr_schedulers += [torch.optim.lr_scheduler.ExponentialLR(
            optimiser, gamma=args.lr_decay, last_epoch=last_epoch)]
...
def show_image_summary(count, foo, in_dict, target_dict, pred_dict):
    ...
    for k, v in in_dict.items():
        ...
        for index in range(v.shape[2]):
            foo.add_images("data/{}_{}".format(k, index),
                           v[:, :3, index], count)
    ...
    for k, v in target_dict.items():
        if len(v.shape) < 5:
            ...
            for index in range(target.shape[2]):
                foo.add_images("data/{}_{}".format(k, str(index)),
                               target[:, :, index].repeat(1, 3, 1, 1), count)

        # show predictions
    for k, v in pred_dict.items():
        pred = v.unsqueeze(2)
        for index in range(pred.shape[2]):
            foo.add_images("pred/{}_{}".format(k, str(index)),
                           pred[:, index].unsqueeze(1).repeat(1, 3, 1, 1), count)


def get_model(cfg):
    model_classes = all_subclasses(BaseNetwork)
    class_index = [cls.__name__ for cls in model_classes].index(
        cfg.MODEL.NETWORK)
...
def get_model_from_args(args, network_models):
    ...
    if 'refine_block' in fn_args:
        class_index = [cls.__name__ for cls in modules].index(
            args.refine_block)
        module_class = list(modules)[class_index]
...
def get_datasets(cfg):
    ...
    try:
        class_index = [cls.__name__ for cls in dataset_classes].index(
            cfg.DATASETS.TRAIN)
    except:
...
def build_dataset(_class, is_train, cfg):
    ...
    cfg_params = dict(list(dict(cfg.items())['INPUT'].items(
    )) + list(dict(cfg.items())['DATASETS'].items()))
    missing_params = []
    for p in fn_args:
        if p in params:
            continue
...
def format_pred(pred):
    ...
    if type(pred) is not list:
        ...
        f_dict = {PRED_LOGITS: pred[0],
                  PRED_SEM_SEG: pred[1], PRED_EMBEDDING: pred[2]}
    else:
        f_dict = None
    return f_dict
